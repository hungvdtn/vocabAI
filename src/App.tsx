      ...
      <AnimatePresence mode="wait">
        ...
        {type === 'roleplay' && (
          <motion.div key="roleplay" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }}>
            <RoleplayGame vocabs={gameVocabs} language={language} onComplete={() => onFinish(gameVocabs.length)} />
          </motion.div>
        )}
      </AnimatePresence>
    </div>
...
function QuizGame({ vocab, allVocabs, onAnswer, onNextStep, language }: { vocab: Vocabulary, allVocabs: Vocabulary[], onAnswer: (c: boolean, ans: string) => void, onNextStep: () => void, language: Language }) {
  ...
  // Tự động phát âm thanh khi hiển thị từ khóa
  useEffect(() => {
...
function FillGame({ vocab, onAnswer, onNextStep, language }: { vocab: Vocabulary, onAnswer: (c: boolean, ans: string, w: string, m: string) => void, onNextStep: () => void, language: Language }) {
  ...
  const check = () => {
    ...
    // Đã gỡ bỏ tính năng phát âm lại từ khóa khi gõ đúng
    setTimeout(onNextStep, isCorrect ? 800 : 2500);
  };
    ...
    <div className="space-y-8">
      <div className="bg-white p-10 md:p-12 rounded-[3rem] shadow-xl border border-slate-100 relative">
        ...
        <h3 className="text-2xl md:text-3xl font-medium leading-loose text-slate-700 text-center mt-8">
          ...
          {/* NẾU KHÔNG TÌM THẤY TỪ NGUYÊN MẪU TRONG CÂU -> ÉP HIỂN THỊ Ô TRỐNG RỒI MỚI IN CÂU */}
          {!hasMatch && (
...
// --- ROLEPLAY GAME (AI CONVERSATION) ---

function RoleplayGame({ vocabs, language, onComplete }: { vocabs: Vocabulary[], language: Language, onComplete: () => void }) {
  ...
  const handleSend = async (textToSend: string) => {
     ...
     try {
        ...
        if (data.candidates && data.candidates[0].content.parts[0].text) {
         ...
         setMessages(prev => [...prev, { role: 'ai', text: "Lỗi kết nối AI. Vui lòng kiểm tra lại API Key hoặc mạng lưới." }]);
     } finally {
         setIsLoading(false);
     }
  };
  ...
  const startListening = () => {
     const SpeechRecognition = (window as any).SpeechRecognition || (window as any).webkitSpeechRecognition;
     if (!SpeechRecognition) {
         alert("Trình duyệt không hỗ trợ nhận diện giọng nói. Vui lòng dùng Chrome, Edge hoặc Safari."); return;
     }
