...
function AssessmentView({ language, onGoToTopics, setIsTestInProgress }: { language: Language, onGoToTopics: () => void, setIsTestInProgress: (status: boolean) => void }) {
  ...
  const generateQuiz = () => {
    ...
    // Thuật toán quét và lấy tối đa 5 câu mỗi trình độ (Tối đa 30 câu)
    standardLevels.forEach(lvl => {
      const wordsInLevel = dict.filter(w => w.level?.toUpperCase() === lvl);
      const questionCount = Math.min(5, wordsInLevel.length); // Lấy tối đa 5 từ/trình độ
      
      if (questionCount > 0) {
        initialScores[lvl] = { correct: 0, total: questionCount };
        const shuffled = wordsInLevel.sort(() => 0.5 - Math.random()).slice(0, questionCount);
        
        shuffled.forEach(w => {
          const correctMeaning = w.vietnamese_meaning || w.meaning;
          let distractors = dict
            .filter(d => d.word !== w.word && (d.vietnamese_meaning || d.meaning))
            .sort(() => 0.5 - Math.random())
            .slice(0, 3)
            .map(d => d.vietnamese_meaning || d.meaning);
          
          // Bổ sung đáp án nhiễu nếu thiếu
          while (distractors.length < 3) {
             distractors.push("Đáp án phụ " + Math.random().toString(36).substring(7));
          }
    ...
    // Kích hoạt khóa bảo vệ chống thoát trang
  ...
  const handleAnswer = (option: string) => {
    ...
    setTimeout(() => {
      if (currentIdx < questions.length - 1) {
        ...
        setPhase('result');
        // Hoàn thành Test -> Gỡ bỏ khóa bảo vệ
        setIsTestInProgress(false);
      }
    }, 1000);
  };
  ...
  const calculateFinalLevel = () => {
    ...
    for (const lvl of evaluatedLevels) {
      ...
      if (acc >= 0.6) { // Yêu cầu trả lời đúng 60% (3/5 câu) ở mức độ đó
        finalLevel = lvl;
      } else {
        break;
      }
    ...
    return (
      <div className="w-full pb-32">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-8">
          <div>
            <h2 className="text-3xl font-bold mb-2">Đánh giá Năng lực Từ vựng</h2>
            <p className="text-slate-500">Kiểm tra vốn từ của bạn theo Khung tham chiếu CEFR (Từ A1 đến C2).</p>
          </div>
        </div>

        <div className="bg-white rounded-[2.5rem] p-8 md:p-12 shadow-sm border border-slate-100 max-w-5xl mx-auto flex flex-col items-center justify-center text-center">
          <div className="w-24 h-24 bg-indigo-50 text-indigo-600 rounded-3xl flex items-center justify-center mb-8 shadow-sm rotate-3">
            <Target size={48} />
          </div>
          <h3 className="text-2xl font-bold text-slate-800 mb-6">Bạn đang ở cấp độ nào?</h3>
          <div className="flex flex-wrap justify-center gap-3 mb-10">
            {['A1', 'A2', 'B1', 'B2', 'C1', 'C2'].map(lvl => (
              <span key={lvl} className="px-6 py-3 bg-slate-50 text-slate-700 font-bold rounded-xl border border-slate-200">{lvl}</span>
            ))}
          </div>
          ...
          <div className="grid md:grid-cols-3 gap-6 w-full mb-12 text-left">
            <div className="bg-indigo-50/50 p-6 rounded-2xl border border-indigo-50">
              ...
              <p className="text-sm text-slate-500 font-medium">Hệ thống chọn 5 câu hỏi ngẫu nhiên cho mỗi cấp độ có trong từ điển (Tối đa 30 câu).</p>
            </div>
  ...
  if (phase === 'quiz') {
    ...
    return (
      <div className="w-full pb-32">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-8">
          <div>
            <h2 className="text-3xl font-bold mb-2">Đang làm bài kiểm tra</h2>
            <p className="text-slate-500">Vui lòng không tải lại hoặc thoát trang để bảo lưu kết quả.</p>
          </div>
          <div className="flex items-center gap-4 bg-white px-6 py-3 rounded-2xl border border-slate-100 shadow-sm">
            <span className="font-bold text-slate-400 uppercase tracking-widest text-xs">Trình độ: <span className="text-indigo-600 text-base">{currentQ.level}</span></span>
            <div className="w-px h-6 bg-slate-200"></div>
            <span className="font-bold text-slate-400 uppercase tracking-widest text-xs">Câu: <span className="text-slate-800 text-base">{currentIdx + 1}/{questions.length}</span></span>
          </div>
        </div>
        ...
        <div className="max-w-4xl mx-auto">
          ...
          <div className="space-y-6">
            <div className="bg-white p-10 md:p-16 rounded-[2.5rem] shadow-sm border border-slate-100 text-center">
              <span className="text-slate-400 font-bold uppercase tracking-widest text-xs mb-4 block">Chọn nghĩa đúng nhất của từ</span>
              <div className="flex items-center justify-center gap-6">
                <h3 className="text-4xl md:text-6xl font-black text-indigo-600 tracking-tight">{currentQ.word}</h3>
                <button onClick={() => handleSpeak(currentQ.word, language)} className="p-4 bg-indigo-50 text-indigo-600 rounded-full hover:bg-indigo-100 hover:scale-110 transition-all shadow-sm"><Volume2 size={28} /></button>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-8">
              {currentQ.options.map((opt: string, i: number) => (
                <button key={i} disabled={!!selectedOption} onClick={() => handleAnswer(opt)}
                  className={cn("p-6 md:p-8 rounded-[2rem] text-left font-bold text-xl transition-all border-2 flex items-center min-h-[120px] shadow-sm", 
                    selectedOption === opt 
    ...
    <div className="w-full pb-32">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-8">
        <div>
          ...
          <p className="text-slate-500">Chúc mừng bạn đã hoàn thành bài khảo sát năng lực.</p>
        </div>
      </div>
      ...
      <div className="grid md:grid-cols-12 gap-8">
        <div className="md:col-span-5 bg-indigo-600 text-white rounded-[2.5rem] p-10 flex flex-col items-center justify-center text-center shadow-xl relative overflow-hidden">
          ...
          <div className="bg-white/20 px-6 py-2 rounded-full text-sm font-bold backdrop-blur-md border border-white/20">
            Mức độ tối đa đo được: {maxAvailableLevel}
          </div>
            ...
            {Object.keys(scores).sort().map(lvl => {
              ...
              return (
                <div key={lvl} className="flex items-center gap-4">
                  <span className="font-bold text-slate-600 text-lg w-10">{lvl}</span>
                  <div className="flex-1 bg-slate-100 h-5 rounded-full overflow-hidden border border-slate-200/50">
