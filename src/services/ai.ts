...
const callWithRetry = async (fn: () => Promise<any>, retries = 3, delay = 1000): Promise<any> => {
  try {
    ...
    if (isRetryable && retries > 0) {
      console.warn(`AI Busy (503/429). Retrying in ${delay}ms... (${retries} left)`);
    ...
    if (isRetryable) {
      throw new Error("Hệ thống AI đang tạm thời quá tải do lượng truy cập lớn. Vui lòng thử lại sau giây lát.");
    }
    throw e;
  }
};
